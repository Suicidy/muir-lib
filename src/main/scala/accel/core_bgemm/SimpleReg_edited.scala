...
abstract class SimpleReg_editedIO(cNum: Int, sNum: Int)(implicit val p: Parameters) extends Module with HasAccelParams with HasAccelShellParams {
  val io = IO(
    new Bundle {
      // val nasti = Flipped(new NastiIO())
      val nasti = Flipped(new AXIMaster(memParams))
...
class SimpleReg_edited(cNum: Int, sNum: Int)(implicit p: Parameters) extends SimpleReg_editedIO(cNum, sNum)(p) {
  ...
  when(canDoWrite) {
    writeAddrReg := io.nasti.aw.bits.addr
  }
  ...
  when(canDoRead) {
    readAddrReg := io.nasti.ar.bits.addr
  }
