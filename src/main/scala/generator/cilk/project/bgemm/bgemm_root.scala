...
class bgemmRootDF(PtrsIn : Seq[Int] = List (16, 16, 16), ValsIn : Seq[Int] = List(), Returns: Seq[Int] = List(), NumChild: Int = 1)
  ...
  for (i <- 0 until NumTiles) {
    ...
    // Parrent to child data interface mapping
...
object bgemmedited extends App {
  val dir = new File("RTL/bgemmedited");
  dir.mkdirs
  implicit val p = new WithAccelConfig ++ new WithTestConfig
  val chirrtl = firrtl.Parser.parse(chisel3.Driver.emit(() => new bgemmRootDF()))
